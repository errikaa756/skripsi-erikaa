...
class ReservasiMejaPayment extends CI_Controller {
    public function __construct()
    {
        ...
        $this->load->model(array(
            'Order_Booking_model' => 'booking',
            'Payment_Booking_model' => 'payment_booking',
            'order_model' => 'order',
            'payment_model' => 'payment',
            'Reservasi_model' => 'reservasi',
            'Reservasi_payment' => 'reservasi_payment'
        ));
    ...
    public function index()
    {
        ...
        $payments['reservasi']= $this->reservasi_payment->get_all_payments();
        $payments['pagination'] = $this->pagination->create_links();
        

        $this->load->view('header', $params);
        $this->load->view('mejapayment/payments', $payments);
    ...
    public function confirm()
    {
        ...
        $payments['booking'] = $this->booking->order_with_bank_payments();
        $payments['reservasi'] = $this->reservasi->order_with_bank_payments();
        ...
        // $payments['bookings_payments']=$this->payment_booking->payment_list();
        $payments['reservasi_payment']=$this->reservasi_payment->payment_list();

        $this->load->view('header', $params);
        $this->load->view('mejapayment/confirm', $payments);
        $this->load->view('footer');
    }
    ...
    public function do_confirm()
    {
        ...
        else
        {
            ...
            //var_dump( $payment );
            // $this->payment_booking->register_payment($order_id, $payment);
            $this->reservasi_payment->register_payment($order_id, $payment);
            
            $this->session->set_flashdata('payment_flash', 'Konfirmasi berhasil dilakukan. Admin akan memverifikasinya dalam waktu 1x24 jam');

            redirect('customer/ReservasiMejaPayment/confirm');
        }
    }
    ...
    public function view($id = 0)
    {
        if ( $this->booking->is_order_exist($id))
        {
            ...
            $order['payment'] = json_decode($booking->payment_data);
            $order['banks'] = $banks;

            $this->load->view('header', $params);
            $this->load->view('mejapayment/view', $order);
            $this->load->view('footer');
        }
        else
        {
