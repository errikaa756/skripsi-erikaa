...
class Meja_payment_model extends CI_Model {
    ...
    public function payment_data($id)
    {
        $payment = $this->db->query("
            SELECT p.*, o.order_number, c.name AS customer, cd.month_year, cd.day, rm.paket, rm.name
            FROM meja_payment p
            JOIN order_meja o
                ON o.id = p.order_id
            JOIN customers c
                ON c.user_id = o.user_id
            JOIN reservasi_item ri
                ON ri.order_id = o.id
            JOIN calendar_days cd
                ON cd.day_id = ri.id_day
            JOIN reservasi_meja rm
                ON rm.id = ri.id_reservasi
            WHERE p.id = '$id'
        ");

        //var_dump($payment->row());
